...
/**
 * File: artwork_report_manager.class.php
 *
 */

/**
 * The artworkReportManagerApp application class
 */
...
class artworkReportManagerApp extends DefaultApplication
{
    ...
    */
    function showEditor($msg = '')
    {
        ...
        if ( $data['status'] ) $statusClause = ' AND ADT.status = ' . q($data['status']);
        ...
        $info['table']  = ARTWORK_DETAILS_TBL . ' AS ADT LEFT JOIN ' . 
                          ORDER_DETAILS_TBL . ' AS ODT ON (ADT.order_details_id=ODT.id) LEFT JOIN ' . 
        ...
        $info['fields'] = array('ODT.id', 'PT.product_code', 'PT.description', 'PT.product_status', 'MT.magazine_abvr', 'ODT.start_month', 
                                'CT.company_name', 'ODT.end_month', 'ODT.customer_id', 'PT.qty_per_unit', 'ADT.shape', 
                                'ADT.position', 'ADT.status', 'ADT.instruction', 'ODT.alternative');
        ...
        return createPage(ARTWORK_REPORT_EDITOR_TEMPLATE, $data);
    }
}
?>