...
/**
 * File: schedule_report_manager.class.php
 *
 */

class artworkReportManagerApp extends DefaultApplication
{
    function run()
    {
    ...
    */
    function showEditor($msg = '')
    {
        ...
        if ( $start_month ) $startMonthClaus = ' AND CAST(SDT.start_month AS UNSIGNED) = '. $start_month;
        if ( $end_month && $end_month != 'Ongoing' ) $endMonthClause = ' AND CAST(SDT.end_month AS UNSIGNED) = ' . $end_month . ' AND SDT.end_month != ' . q('Ongoing');
        if ( $end_month && $end_month == 'Ongoing' ) $endMonthClause = ' AND SDT.end_month = ' . q('Ongoing');
        ...
        $info['table']  = SCHEDULE_DETAILS_TBL . ' AS SDT LEFT JOIN ' . 
                          ORDER_DETAILS_TBL . ' AS ODT ON (SDT.order_details_id=ODT.id) LEFT JOIN ' . 
                          PRODUCT_TBL . ' AS PT ON (ODT.product_id=PT.id) LEFT JOIN ' . 
                          MAGAZINES_TBL . ' AS MT ON (ODT.magazine_code=MT.id) LEFT JOIN ' . 
                          CUSTOMERS_TBL . ' AS CT ON (ODT.customer_id = CT.id)';
        $info['debug']  = false;
        $info['where']  = '1' . $startMonthClaus . $endMonthClause . $statusClause . $magazineClause . $customerClause . 
                                $productClause . ' ORDER BY ODT.id DESC';
        $info['fields'] = array('ODT.id', 'PT.product_code', 'PT.description', 'PT.product_status', 'MT.magazine_abvr', 'SDT.start_month', 
                                'SDT.instruction', 'CT.company_name', 'SDT.end_month', 'ODT.customer_id', 'PT.qty_per_unit', 'SDT.position', 
                                'SDT.file_name', 'SDT.date_sensitive', 'ODT.alternative', 'ODT.status');
        ...
        return createPage(SCHEDULE_REPORT_EDITOR_TEMPLATE, $data);
    }
}
?>