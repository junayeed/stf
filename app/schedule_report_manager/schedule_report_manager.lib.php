<?php
    ...
    {
        $headerArray = array('A'=>'Order #', 'B'=>'Customer Name', 'C'=>'Megazine', 'D'=>'Start Month', 'E'=>'End Month', 'F'=>'Size', 
                             'G'=>'Position', 'H'=>'Dated', 'I'=>'File Name', 'J'=>'Instruction');
        ...
        if ($data['cmd'] == 'excel')
        {
            ...
            $filename  = 'excel_schedule_inventory_report.xlsx';
        }
        ...
        $objSheet->mergeCells('A'.$row.':J'.$row);
        $objSheet->getStyle('A'.$row.':J'.$row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        $objSheet->getStyle('A'.$row.':J'.$row)->getFont()->setBold(true)->setSize(14);
        $objSheet->getCell('A'.$row)->setValue('Discover Magazine Ltd.');
        
        $row++;
        if ( $data['magazine'] == 'All') $magazine = 'All Magazine';
        else if ($data['magazine'] == 'No') $magazine = 'No Magazine';
        ...
        $objSheet->getStyle('A'.$row.':J'.$row)->getFont()->setBold(true)->setSize(10);
        $objSheet->getStyle('A'.$row.':J'.$row)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        ...
        {
            $objSheet->getStyle('A'.$row.':J'.$row)->getFont()->setSize(10);
            $objSheet->getCell('A'.$row)->setValue($oValue->id);
            $objSheet->getCell('B'.$row)->setValue($oValue->company_name);
            $objSheet->getCell('C'.$row)->setValue($oValue->magazine_abvr);
            $objSheet->getCell('D'.$row)->setValue($oValue->start_month);
            $objSheet->getCell('E'.$row)->setValue($oValue->end_month);
            $objSheet->getCell('F'.$row)->setValue($oValue->qty_per_unit);
            $objSheet->getCell('G'.$row)->setValue($oValue->position);
            $objSheet->getCell('H'.$row)->setValue($oValue->date_sensitive);
            $objSheet->getCell('I'.$row)->setValue($oValue->file_name);
            $objSheet->getCell('J'.$row)->setValue($oValue->instruction);
            $objSheet->getStyle('A'.$row.':I'.$row)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
